...
- 🔔 **تذكيرات** عبر `expo-notifications` على native (و preview آمن على web).
...
6 اختبارات خضراء تغطي: عقد التصنيفات، السكيما المرجعية، عقد المساعد (intents + dates)، وجدولة التذكيرات. اختبار `auth.logout` متعمد `skip` حتى يكتمل ربط المصادقة.
