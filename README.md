...
- 📸 **حلقة اللقطات (screenshot loop)**: التطبيق كيقلب على أحدث اللقطات فـ معرض الهاتف، تقدر تحفظها فالمكتبة بضغطة، ولا «حلّلها ✦» — vision analysis فـ server (`content.analyzeImage`) كيخرج OCR + نوع المحتوى (حديث/فيديو/منشور...) + tags + التصنيف + أفضل وقت للمراجعة.
- ⏰ **محرك التوقيت**: ملي كتختار «خلي التطبيق يقرر»، `shared/timing-engine.ts` كيتعلم من عاداتك — الفترة المعتادة بين الالتقاط والمراجعة، ساعة النشاط المفضلة، والخيار الأكثر استخداماً — و كيبرمج التذكير عليها.
